...
func _ready() -> void:
	...
	get_parent().get_parent().connect("ship_selected", _show_build_view)
	get_parent().get_parent().connect("ship_deselected", _hide_build_view)
