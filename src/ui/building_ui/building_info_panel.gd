...
func show_info_panel(pos: Vector2, building_data: BuildingResource):
	...
	match building_data.type:
		...
		EnumAutoload.BuildingType.FOOD:
			desc_label.text = "Can produce {n_food} units of Food per day.".format({"n_food": building_data.resource_prod.food})
		EnumAutoload.BuildingType.WATER:
			desc_label.text = "Can produce {n_water} units of Water per day.".format({"n_water": building_data.resource_prod.water})
		EnumAutoload.BuildingType.AIR:
			desc_label.text = "Can produce {n_air} units of Oxygen per day.".format({"n_air": building_data.resource_prod.air})
		EnumAutoload.BuildingType.METAL:
			desc_label.text = "Can produce {n_metal} units of Metal per day.".format({"n_metal": building_data.resource_prod.metal})
